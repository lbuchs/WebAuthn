...
class WebAuthn {
    ...
    public function processCreate($clientDataJSON, $attestationObject, $challenge) {
        ...
        if (!$attestationObject->validateRpIdHash($this->_rpIdHash)) {
            throw new Exception('invalid rpId hash');
        }
        ...
        $data = new stdClass();
        $data->credentialId = base64_encode($attestationObject->getAuthenticatorData()->getCredentialId());
    ...
    private function _createChallenge($length = 32) {
        if (!$this->_challenge) {
            ...
            if (!$crypto_strong) {
                throw new Exception('cannot create crypto-strong random bytes.');
            }
        }
        return $this->_challenge;
    }
